...
const calculatePosition = (duck, index, total, centerX, centerY, radiusX, radiusY, pairs, danceAngles, angleOffset) => {
  ...
  // Adjust radius based on number of ducks
  let adjustedRadiusX = radiusX;
  let adjustedRadiusY = radiusY;
  
  if (total > 50) {
    // Increase radius to prevent overcrowding
    const scaleFactor = 1 + ((total - 50) / 100);
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (value === '' || (Number(value) >= 0 && Number(value) <= 1000)) {
                        setClassSize(value);
                        // Special hint when they enter 23
                        if (value === '23') {
                          // Could add a special animation or highlight here
                        }
                      }
                    }}
                    ...
                    onChange={(e) => {
                      ...
                      if (value === '' || (Number(value) >= 0 && Number(value) <= 100)) {
                        ...
                        if (value.split('.').length <= 2 && value.length <= 6) {
                          ...
                          // Special hint when they enter 50
                          if (value === '50') {
                            // Could add a special animation or highlight here
                          }
                        }
