...
const DuckPond = () => {
  ...
  const calculateProbability = (n) => {
    if (!n || isNaN(n) || n <= 0) return 0; // Handle invalid inputs
    ...
    const result = (1 - prob) * 100;
    // Use toFixed only if result is valid, handle edge cases
    if (isNaN(result)) return 0;
    
    // Precision logic: more decimals for near-100% values
    return result > 99 ? result.toFixed(4) : result.toFixed(1);
  };
  

  const calculateDifference = () => {
    const actualProb = calculateProbability(parseInt(classSize));
    const guess = parseFloat(probabilityGuess) || 0;
  
    return Math.abs(actualProb - guess).toFixed(1);
  };
